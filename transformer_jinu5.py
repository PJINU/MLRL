...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        self.encoder_student = copy.deepcopy(self.encoder)
    ...
    def get_target_block_prac(self, target_encoder, src, target_ratio, num_blocks):
        ...
        num_patches_block = int(num_patches * target_ratio)
        ...
        # target_block = torch.zeros((num_blocks, num_patches_block, src.shape[1], src.shape[2])).cuda() # [16, 25, 64]
        target_block = torch.zeros((num_blocks, num_patches_block, src.shape[1], src.shape[2])).cuda()
        target_patches = []
        all_patches = []
        ...
        for z in range(num_blocks):
            patches = []    
            src_index = torch.tensor([i for i in range(src.shape[0]) if i != 0]).cuda()
            target_index = torch.randperm(src_index.size(0))[:num_patches_block].cuda()
            target_block[z] = src[target_index, :, :]
            all_patches.append(target_index)

        # ipdb.set_trace()
        # all_patches = sorted(set(torch.cat(all_patches)))
        all_patches = torch.sort(torch.unique(torch.cat(all_patches))).values
    ...
    def forward(self, src, 
        ...
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)  # (L, batch_size, d) / [76, 32, 256]
        ...
        # target block
            target_ratio = 0.15
            target_block, target_patches, target_all_patches = self.get_target_block_prac(self.encoder, src, target_ratio, num_blocks)
            m, tl, bz, d= target_block.shape # m : num of blocks, vl : length of target, bz : batch size, d : dimension
        
        
            # context block
            # ipdb.set_trace()
            src_index = [i for i in range(memory_local.shape[0])]
            context_index = torch.tensor([i for i in src_index if i not in target_all_patches]).cuda()
            # context_index = [0] +  sorted(random.sample(context_index, int(len(context_index)*0.85)))
            context_block = memory_local[context_index, :, :]
            # ipdb.set_trace()
            context_mask = mask[:, :context_block.shape[0]]
            ...
            noise_st = 0.01
            noise_mean = 0
            # noise = torch.randn(context_block.size()).cuda() * noise_st + noise_mean
            noise = torch.randn(context_block.size(0), 1, context_block.size(2)).cuda() * noise_st + noise_mean
            noise = noise.expand_as(context_block)
            ...
            for i in range(num_blocks):
                tgt_rep = self.mask_token.repeat(tl, bz, 1)
                
                output = self.predictor(tgt_rep, context_block, memory_key_padding_mask=context_mask)
            ...
            return hs, references, memory_local, memory_global, prediction_blocks_rep.view(-1,bz,d).permute(1,0,2), target_block.view(-1, bz, d).permute(1,0,2)
