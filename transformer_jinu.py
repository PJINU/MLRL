...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        decoder_norm = nn.LayerNorm(d_model)
        self.predictor = nn.TransformerDecoder(decoder_layer, num_decoder_layers)
    ...
    def forward(self, src, 
        ...
        context_index = [0] +  sorted(random.sample(context_index, int(len(context_index)*0.8)))
        ...
        for i in range(num_blocks):
            tgt = self.mask_token.repeat(tl, bz, 1)
        ...
        # ipdb.set_trace()
        return prediction_blocks_rep.view(-1,bz,d).permute(1,0,2), prediction_blocks_span.view(-1,bz,d).permute(1,0,2), target_block.view(-1, bz, d).permute(1,0,2), memory_local
