    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if epoch_i > -1:
            ...
            for batch_idx, batch in tqdm(enumerate(train_loader),
                ...
                targets_span = torch.zeros(outputs['pred_spans'].shape)
                for i, t in enumerate(targets['span_labels']):
                    if t['spans'].shape[0] > 1:
                        rand_idx = random.randrange(0, t['spans'].shape[0])
                        targets_span[i] = t['spans'].cpu()[rand_idx].repeat(4,1)
                    else:
                        targets_span[i] = t['spans'].cpu().repeat(4,1)
                

                targets_span = targets_span.cuda()
                # ipdb.set_trace()
                
                loss_mse = criterion['mse'](outputs['pred_reps'], target_block)
                loss_l1 = criterion['l1'](outputs['pred_spans'], targets_span)
                
                losses = (loss_mse + loss_l1) / 2
                
                train_loss_total += losses
            ...
            num_iter = len(train_loader)
            print('')
            console.print(f'[red]train_loss_total[/red] : {train_loss_total / num_iter} / [red]train_loss_mse[/red] : {train_loss_mse / num_iter} / [red]train_loss_l1[/red] : {train_loss_l1 / num_iter}')
...
def start_training():
    ...
    criterion = {'mse' : criterion_mse, 'l1' : criterion_l1}
