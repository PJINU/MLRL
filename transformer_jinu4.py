...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        self.t2v_encoder_teacher = TransformerEncoder(t2v_encoder_layer, num_encoder_layers, encoder_norm)
        self.t2v_encoder_student = copy.deepcopy(self.t2v_encoder_student)
    ...
    def get_target_block(self, target_encoder, src, video_length, aspect_ratio, scale, num_blocks):
        # ipdb.set_trace()

        target_encoder = target_encoder.eval()
        # patch_h, patch_w = patch_dim
        src = target_encoder(src, src_key_padding_mask=mask, pos=pos_embed, video_length=video_length)        
        
        src = self.norm(src)
        
        target_src = src[1:video_length + 1]
        
        num_patches = target_src.shape[0]
        ...
        target_block = torch.zeros((num_blocks, block_h*block_w, target_src.shape[1], target_src.shape[2])).cuda() # [16, 25, 64]
        target_patches = []
        all_patches = []
        ...
        for z in range(num_blocks):
            ...
            target_block[z] = target_src[patches, :, :] # x[:, patches, :].shape = [16, 25, 64]
            # ipdb.set_trace()
    ...
    def forward(self, src, 
        ...
        if train_type == 'representation':
            ...
            # target_src = src + pos_embed
            # ipdb.set_trace()
 
            target_block, target_patches, target_all_patches = self.get_target_block(self.t2v_encoder_teacher, src, pos_embed, video_length, target_aspect_ratio, target_scale, num_blocks)
            # (L, batch_size, d) / [98, 32, 256]
            # print('after encoder : ',src.shape)
            target_src = src[1:video_length + 1] # video_length=75 / src = [76, 32, 256] (global token 포함)
            ...
            # target_block, target_patches, target_all_patches = self.get_target_block(self.encoder_teacher, target_src, target_aspect_ratio, target_scale, num_blocks)
            ...
            context_index = [0] +  sorted(random.sample(context_index, int(len(context_index)*0.55)))
