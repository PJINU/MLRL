...
class QDDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        self.span_embed = MLP(hidden_dim, hidden_dim*4, span_pred_dim, 3)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        #                                                         ) 
        pred_rep, pred_span, target_block, memory_local = self.transformer(src, 
                                                                ~mask, 
                                                                self.query_embed.weight, 
                                                                pos, 
                                                                video_length=video_length,
        ...
        # ipdb.set_trace()
        # outputs_class = self.class_embed(pred_span)  # (#layers, batch_size, #queries, #classes) / shape : [2, 32, 10, 2]
        # reference_before_sigmoid = inverse_sigmoid(reference) # shape : [2, 32, 10, 2]
        pred_span = self.span_embed(pred_span) # shape : [2, 32, 10, 2]
        # outputs_coord = tmp + reference_before_sigmoid # shape : [2, 32, 10, 2]
        # if self.span_loss_type == "l1":
        #     outputs_coord = outputs_coord.sigmoid()
        # out = {'pred_logits': outputs_class[-1], 'pred_spans': outputs_coord[-1]}
        
        out = {'pred_reps' : pred_rep, 'pred_spans' : pred_span}
