...
from qd_detr.transformer_jinu5 import build_transformer
...
class QDDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        # Block config
        self.num_blocks = 4
        # self.target_aspect_ratio = [0.75, 1.5]
        self.target_aspect_ratio = [0.65, 1.6]
        # self.target_scale = [0.15, 0.2]
        self.target_scale = [0.25, 0.2]
        self.context_aspect_ratio = 1
        self.context_scale = [0.85, 1.0]
        self.patch_size = 75
        self.num_tokens = 75
        self.momentum = 0.996
        self.m_start_end = (0.996, 1)
        
        
        

    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None, train_type=None, stage=None):
        """The forward expects two tensors:
               - src_txt: [batch_size, L_txt, D_txt] ; ex) [32, 22, 512]
               - src_txt_mask: [batch_size, L_txt], containing 0 on padded pixels, ; ex) [32, 22]
                    will convert to 1 as padding later for transformer
               - src_vid: [batch_size, L_vid, D_vid] ; ex) [32, 75, 2818]
               - src_vid_mask: [batch_size, L_vid], containing 0 on padded pixels, ; ex) [32, 75]
                    will convert to 1 as padding later for transformer
        ...
        src_vid = self.input_vid_proj(src_vid) # shape : [32, 75, 2818] -> [32, 75, 256]
        src_txt = self.input_txt_proj(src_txt) # shape : [32, 22, 512] -> [32, 22, 256]
        src = torch.cat([src_vid, src_txt], dim=1)  # (bsz, L_vid+L_txt, d) / shape : [32, 97, 256]
        mask = torch.cat([src_vid_mask, src_txt_mask], dim=1).bool()  # (bsz, L_vid+L_txt) / shape : [32, 97]
        # TODO should we remove or use different positional embeddings to the src_txt?
        pos_vid = self.position_embed(src_vid, src_vid_mask)  # (bsz, L_vid, d) ; [32, 75, 256]
        pos_txt = self.txt_position_embed(src_txt) if self.use_txt_pos else torch.zeros_like(src_txt)  # (bsz, L_txt, d) / shape : [32, 22, 256]
        # pos_txt = torch.zeros_like(src_txt)
        # pad zeros for txt positions
        pos = torch.cat([pos_vid, pos_txt], dim=1) # shape : [32, 97, 256]
        # (#layers, bsz, #queries, d), (bsz, L_vid+L_txt, d)

        # for global token
        mask_ = torch.tensor([[True]]).to(mask.device).repeat(mask.shape[0], 1) # shape : [32, 1]
        mask = torch.cat([mask_, mask], dim=1) # shape : [32, 98]
        src_ = self.global_rep_token.reshape([1, 1, self.hidden_dim]).repeat(src.shape[0], 1, 1) # shape : [32, 1, 256]
        src = torch.cat([src_, src], dim=1) # shape : [32, 98, 256]
        pos_ = self.global_rep_pos.reshape([1, 1, self.hidden_dim]).repeat(pos.shape[0], 1, 1) # shape : [32, 1, 256]
        pos = torch.cat([pos_, pos], dim=1) # shape : [32, 98, 256]
        ...
        if stage == 'train':
            hs, reference, memory, memory_global, prediction_block, target_block, = self.transformer(src, 
                                                        ~mask, 
                ...
                # ipdb.set_trace()
                # hs shape : [2, 32, 10, 256]
                # reference : [2, 32, 10, 2]
                # memory : [32, 75, 256]
                # memory_global : [32, 256]
                # ipdb.set_trace()
            outputs_class = self.class_embed(hs)  # (#layers, batch_size, #queries, #classes) / shape : [2, 32, 10, 2]
            reference_before_sigmoid = inverse_sigmoid(reference) # shape : [2, 32, 10, 2]
            tmp = self.span_embed(hs) # shape : [2, 32, 10, 2]
            outputs_coord = tmp + reference_before_sigmoid # shape : [2, 32, 10, 2]
            ...
            _, _, memory_neg, memory_global_neg = self.transformer(src_neg, ~mask_neg, self.query_embed.weight, pos_neg, train_type='hl', video_length=video_length)
            ...
            return out, prediction_block, target_block
    
        else:
            hs, reference, memory, memory_global= self.transformer(src, 
                                                        ~mask, 
                                                        self.query_embed.weight, 
                                                        pos, 
                                                        video_length=video_length,
