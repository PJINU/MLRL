...
def train(model, criterion, rep_train_loss, optimizer, optimizer_rep, lr_scheduler, lr_scheduler_rep, train_dataset, val_dataset, opt):
    ...
    # --------------- Start Representation Train --------------- 
    
    for epoch_i in trange(start_epoch, opt.n_epoch_rep, desc="Representation Epoch"):
        
        # Train
        if epoch_i > -1:
            # train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer)
            logger.info(f"[Representation Epoch {epoch_i+1}]")
            model.train()
            # criterion.train()
            ...
            num_training_examples = len(rep_train_loader) # 226
            timer_dataloading = time.time()
            # Iteration
            ...
            for batch_idx, batch in tqdm(enumerate(rep_train_loader),
                                            desc="Representation Training Iteration",
                                            total=num_training_examples):
                time_meters["dataloading_time"].update(time.time() - timer_dataloading)
                ...
                timer_start = time.time()
                # ipdb.set_trace()        
                model_inputs['train_type'] = 'representation'
                outputs, target_block = model(**model_inputs) # outputs : ['pred_logits', 'pred_spans', 'saliency_scores', 'saliency_scores_neg', 'video_mask', 'aux_outputs]
                # outputs['pred_logits'] = [32, 10, 2]
                # outputs['pred_spans'] = [32, 10, 2]
                # outputs['saliency_scores'] = [32, 75]
                # outputs['saliency_scores_neg'] = [32, 75]
                ...
                loss_rep = rep_train_loss(outputs, target_block)
                
                train_loss_total += loss_rep.item()
            ...
            num_iter = len(train_loader)
            print('')
            console.print(f'[red]train_loss_total[/red] : {train_loss_total / num_iter}')
            ...
            # ipdb.set_trace()
            # print('gg')
            student_model = model.transformer.encoder_student.eval()
            teacher_model = model.transformer.encoder_teacher.eval()
            with torch.no_grad():
                for student_param, teacher_param in zip(student_model.parameters(), teacher_model.parameters()):
                    teacher_param.data.mul_(other=momentum).add_(other=student_param.data, alpha=1-momentum)
            
            momentum += (m_start_end[1] - m_start_end[0])
            
        # evaluation

        model.eval()
        rep_val_loss = 0.0
        num_validation_examples = len(rep_val_loader)
        with torch.no_grad():
            for batch_idx, batch in tqdm(enumerate(rep_val_loader), 
                                        desc="Representation Validation Iteration",
                                        total=num_validation_examples):
                    
                model_inputs, targets = prepare_batch_inputs(batch[1], opt.device, non_blocking=opt.pin_memory)
                    
                model_inputs['train_type'] = 'representation'
                outputs, target_block = model(**model_inputs) 

                loss_rep = rep_train_loss(outputs, target_block)
                rep_val_loss += loss_rep.item()
            
        rep_val_loss /= len(rep_val_loader)
        num_val_iter = len(rep_val_loader)
        
        console.print(f'[red]train_loss_total[/red] : {train_loss_total / num_iter} // [red]rep_val_loss : {rep_val_loss / num_val_iter}')
                    

        lr_scheduler_rep.step()
        
        if rep_val_loss < best_loss:
            best_loss = rep_val_loss
            epochs_no_improve = 0
        ...
        if epochs_no_improve >= early_stopping_patience:
            print(f'Representation Early stopping after {epoch_i + 1} ')
            break
